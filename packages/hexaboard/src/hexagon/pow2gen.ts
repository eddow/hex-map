...
vec4 tColor(sampler2D textures[3], int i, vec2 vUv) {
	if(i == 0)
		return texture2D(textures[0], vUv);
	if(i == 1)
		return texture2D(textures[1], vUv);
	if(i == 2)
		return texture2D(textures[2], vUv);
...
/*
	...
	vec4 color1 = tColor(textures, int(0), vUv[0]);
	vec4 color2 = tColor(textures, int(1), vUv[1]);
	vec4 color3 = tColor(textures, int(2), vUv[2]);
*/
...
> extends HexPow2Gen<Point> {
	...
	triangle(ndx: [number, number, number], side: number): Mesh {
		...
		geometry.setAttribute('textureIdx', new Int16BufferAttribute([0, 1, 2, 0, 1, 2, 0, 1, 2], 3))
